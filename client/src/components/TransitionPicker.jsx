  ...
  return (
    <div ref={rootRef} className="relative flex items-center shrink-0">
      <button
        onClick={() => setOpen((o) => !o)}
        className={[
          'mx-0.5 my-2 w-7 h-16 rounded-md text-xs flex items-center justify-center border',
          isNone
      ...
      {open && (
        <div className="absolute z-30 top-full left-1/2 -translate-x-1/2 mt-1 w-52 p-3 rounded-xl bg-slate-900 border border-slate-700 shadow-xl">
          <label className="block text-[11px] text-slate-400 mb-1">Transition</label>
          <select
            value={value.type}
            onChange={(e) => setType(e.target.value)}
            className="w-full mb-2 px-2 py-1.5 rounded-md bg-slate-800 border border-slate-700 text-xs text-slate-200"
          >
